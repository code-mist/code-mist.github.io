...
这三个操作符在 C++ 中有不同的作用。让我为你详细解释：

#### `.` 操作符（点操作符）

**作用**：用于访问对象的成员（数据成员或成员函数）。
...
**作用**：用于访问**命名空间、类、枚举**中的成员或全局作用域中的符号。
...
Math::PI = 314;       // 访问静态数据成员
Math::calculate();    // 访问静态成员函数
```
...
void MyClass::myFunction() {  // 使用 :: 在类外定义
    cout << "Hello";
}
```
...
Person p;
Person* ptr = &p;      // 指向对象的指针

ptr->name = "Bob";     // 访问指针所指对象的数据成员
ptr->greet();          // 调用指针所指对象的成员函数
```

**等价写法**：
```cpp
ptr->name;      // 使用箭头操作符
(*ptr).name;    // 等价：先解引用，再用点操作符
```
...
| 操作符 | 用于                               | 示例                                       |
| ------ | ---------------------------------- | ------------------------------------------ |
| `.`    | 直接对象或引用                     | `object.member`                            |
| `::`   | 命名空间、类的静态成员、全局作用域 | `ClassName::member` 或 `namespace::symbol` |
| `->`   | 指针                               | `pointer->member`                          |
...
int Student::count = 0;  // 使用 :: 初始化静态成员

int main() {
    Student s;
    s.name = "Tom";       // 使用 . 访问对象成员
    s.study();            // 使用 . 调用成员函数
    
    Student* ps = &s;
    ps->name = "Jerry";   // 使用 -> 访问指针所指对象的成员
    ps->study();          // 使用 -> 调用成员函数
    
    Student::count = 1;   // 使用 :: 访问静态成员
...
| **目的**     | 初始化对象成员变量和资源                                     | 清理对象占用的资源                                           |
| **函数名**   | 与类名相同                                                   | `~类名`                                                      |
| **参数**     | 可以有参数（支持重载）                                       | 无参数                                                       |
| **返回值**   | 无返回值                                                     | 无返回值                                                     |
| **个数**     | 可以有多个（重载）                                           | 只能有一个                                                   |
| **调用次数** | 每次创建对象调用一次                                         | 每个对象销毁时调用一次                                       |
| 虚函数       | 通常不是虚函数                                               | 应该是虚函数-多态                                            |
| 作用         | 初始化对象=创建一个对象时，构造函数会自动执行，为成员变量赋予初值，并分配必要的资源（如内存、文件、数据库连接等） | 清理对象=当对象生命周期结束（超出作用域或被显式删除）时，析构函数自动执行，释放对象占用的动态内存、关闭文件、断开数据库连接等。 |
| 场景         | 初始化成员变量的初值 验证输入参数的有效性 分配动态内存 打开文件或建立网络连接 | 释放通过 `new` 分配的动态内存 关闭打开的文件 断开数据库连接 释放其他系统资源 |
|              |                                                              | 若对象中包含动态分配的资源，必须定义析构函数进行清理，否则会导致内存泄漏 |
