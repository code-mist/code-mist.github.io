...
| 文件类型          | 应包含的内容                                                 | 不应包含的内容                                               |
| :---------------- | :----------------------------------------------------------- | :----------------------------------------------------------- |
| **`.h` 头文件**   | - 类/结构体声明<br>- 函数原型、`extern` 全局变量声明<br>- 类型别名（`typedef` / `using`）<br>- 常量声明（`constexpr`、`extern const`）<br>- 内联函数/模板的定义（必须放在头文件）<br>- 宏定义（谨慎使用） | - 非内联/非模板的函数实现<br>- 静态成员变量的定义（应在 `.cpp` 中）<br>- 全局变量的定义（`int g_val;`） |
| **`.cpp` 源文件** | - 头文件中声明的非内联函数实现<br>- 类静态成员变量的定义与初始化<br>- 全局变量的定义<br>- 仅本文件需要的私有函数/辅助函数（`static` 或匿名命名空间） | - 重复的头文件保护宏<br>- 不属于本模块的公开声明             |

> ✅ **核心原则**：头文件用于“告诉外界有什么”，源文件用于“具体怎么做”。避免在头文件中放置实现细节（除模板/内联外），以降低编译依赖和减少重编译开销。

#### 反例与正例

**❌ 错误：在 `math_utils.h` 中定义非内联函数**
```cpp
// math_utils.h
#ifndef MATH_UTILS_H
#define MATH_UTILS_H

int add(int a, int b) { return a + b; }   // 错误：非内联定义不应放在头文件
