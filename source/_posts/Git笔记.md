...
```markdown
- 注释：以#开头的行被视为注释。
- 普通文件和目录：直接写出文件或目录的名称，如debug.log或temp/。
- 通配符：使用*匹配任意数量的字符，如*.log将匹配所有以.log结尾的文件。
- 目录：在目录名后加上/，如node_modules/。
- 否定模式：以!开头的模式表示排除特定文件，如!important.txt。
```
...
2.合并其他分支过来：`git merge 要合并的分支`

3.删除合并后的分支指针：`git branch -d 要合并的分支`
...
创建：公司自己服务器 / 第三方托管平台（GitLab，GitHub...）
...
效果：在运行命令所在文件夹，生成 项目文件夹（包含版本库，并映射到暂存区和工作区）
